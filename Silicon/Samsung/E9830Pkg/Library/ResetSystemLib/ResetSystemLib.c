...
#include "PMUCal9830.h"

INT32 
PMUCalLocalDisable (UINT32 pd_id)
{
	INT32 ret = 0;
	...
	ret = PMUCalRaeHandleSeq(pmucal_pd_list[pd_id].off,
  ...
  )
{
  DEBUG ((EFI_D_ERROR, "ResetSystemLib: Shutting down!"));
  	...
  	UINT32 i;
	UINT32 start = 0, end = PMUCAL_NUM_PDS;
	INT32 ret;
	UINT32 chip_rev;
	...
	for (i = start; i < end; i++) {
		DEBUG((EFI_D_ERROR, "%s power off... ", pmucal_pd_list[i].name));
		ret = PMUCalLocalDisable(i);
