...
void dsim_hex_dump(void __iomem *base_addr, int size)
{
	int i, j;
	int height, num_word;
	...
	num_word = size / 4;
	for (i = 0; i < height; i++) {
		dsim_info("[0x%8p] ", (base_addr + i * 32));
		for (j = 0; j < 8; j++) {
			if (num_word <= 0) break;
			dsim_info("%08x ", __raw_readl(base_addr + i * 32 + j * 4));
			num_word--;
		}
		dsim_info("\n");
		if (num_word <= 0) break;
...
void dpu_sysreg_dphy_reset(void __iomem *sysreg, u32 dsim_id, u32 rst)
{
	u32 old = readl(sysreg + DISP_DPU_MIPI_PHY_CON);
	u32 val = rst ? ~0 : 0;
	u32 mask = dsim_id ? M_RESETN_M4M4_MODULE_MASK : M_RESETN_M4M4_TOP_MASK;
...
//#endif
	...
	if (dsim->state == DSIM_STATE_INIT) {
		if (dsim_reg_get_link_clock(dsim->id)) {
			dsim_info("dsim%d is already enabled\n", dsim->id);
			return;
		}
	}
#endif
	/* DPHY reset control from SYSREG(0) */
	dpu_sysreg_select_dphy_rst_control((u32 *)dsim->res.ss_regs, dsim->id, 0);
...
void __dsim_dump(u32 id, struct dsim_regs *regs)
{
	...
	dsim_hex_dump(regs->regs, 0x124);

	dsim_info("=== DSIM %d DPHY SFR DUMP ===\n", id);
	/* DPHY dump */
	dsim_hex_dump(regs->phy_regs_ex + 0x0000, 0x14);
	/* PLL */
	dsim_hex_dump(regs->phy_regs + 0x0000, 0x44);
	/* MC */
	dsim_hex_dump(regs->phy_regs + 0x0200, 0x48);
	dsim_hex_dump(regs->phy_regs + 0x02E0, 0x10);
	/* MD0 */
	dsim_hex_dump(regs->phy_regs + 0x0300, 0x70);
	dsim_hex_dump(regs->phy_regs + 0x03C0, 0x40);
	/* MD1 */
	dsim_hex_dump(regs->phy_regs + 0x0400, 0x70);
	dsim_hex_dump(regs->phy_regs + 0x04C0, 0x40);
	/* MD2 */
	dsim_hex_dump(regs->phy_regs + 0x0500, 0x70);
	dsim_hex_dump(regs->phy_regs + 0x05C0, 0x40);
	/* MD3 */
	dsim_hex_dump(regs->phy_regs + 0x0600, 0x48);

	dsim_hex_dump(regs->phy_regs + 0x06C0, 0x20);
