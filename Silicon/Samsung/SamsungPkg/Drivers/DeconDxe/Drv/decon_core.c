...
/* logo/font framebuffer overlay was intentionally dropped for this DXE
 * port -- only a single (LOGO_DPP) framebuffer window is created; see
 * decon_show_buffer()/display_drv_init() below. */
#define LUT_COLOR_1	0xFFFFFFFF
...
/*
 * Base address of the single framebuffer this DXE creates. Set by
 * DeconDxeEntry() (via DeconSetFramebufferBase()) to the physical
 * address of the UEFI-allocated buffer before display_drv_init() runs.
 */
unsigned int win_fb0;

void decon_set_framebuffer_base(unsigned int addr)
{
	win_fb0 = addr;
}
...
int display_drv_init(void)
{
	...
	/* DPP0 G0 for the (single) framebuffer -- no second/font DPP is
	 * probed here; the GOP driver that consumes this framebuffer is
	 * expected to live in a separate module. */
