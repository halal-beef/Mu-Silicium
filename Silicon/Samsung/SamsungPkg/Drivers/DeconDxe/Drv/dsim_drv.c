...
static void dsim_long_data_wr(struct dsim_device *dsim, unsigned long d0, u32 d1)
{
	...
	for (data_cnt = 0; data_cnt < d1; data_cnt += 4) {
		...
		if ((d1 - data_cnt) < 4) {
			if ((d1 - data_cnt) == 3) {
				payload = *(u8 *)(uintptr_t)(d0 + data_cnt) |
				    (*(u8 *)(uintptr_t)(d0 + (data_cnt + 1))) << 8 |
					(*(u8 *)(uintptr_t)(d0 + (data_cnt + 2))) << 16;
			dsim_dbg("count = 3 payload = %x, %x %x %x\n",
				payload, *(u8 *)(uintptr_t)(d0 + data_cnt),
				*(u8 *)(uintptr_t)(d0 + (data_cnt + 1)),
				*(u8 *)(uintptr_t)(d0 + (data_cnt + 2)));
			} else if ((d1 - data_cnt) == 2) {
				payload = *(u8 *)(uintptr_t)(d0 + data_cnt) |
					(*(u8 *)(uintptr_t)(d0 + (data_cnt + 1))) << 8;
			dsim_dbg("count = 2 payload = %x, %x %x\n", payload,
				*(u8 *)(uintptr_t)(d0 + data_cnt),
				*(u8 *)(uintptr_t)(d0 + (data_cnt + 1)));
		...
		/* send 4bytes per one time. */
			...
			payload = *(u8 *)(uintptr_t)(d0 + data_cnt) |
				(*(u8 *)(uintptr_t)(d0 + (data_cnt + 1))) << 8 |
				(*(u8 *)(uintptr_t)(d0 + (data_cnt + 2))) << 16 |
				(*(u8 *)(uintptr_t)(d0 + (data_cnt + 3))) << 24;

			dsim_dbg("count = 4 payload = %x, %x %x %x %x\n",
				payload, *(u8 *)(uintptr_t)(d0 + data_cnt),
				*(u8 *)(uintptr_t)(d0 + (data_cnt + 1)),
				*(u8 *)(uintptr_t)(d0 + (data_cnt + 2)),
				*(u8 *)(uintptr_t)(d0 + (data_cnt + 3)));
