...
void s6e3hab_lcd_set_resol(unsigned int id, struct exynos_panel_info *lcd)
{
	...
	/* Scaler settings*/
	if (dsim_wr_data(id, MIPI_DSI_DCS_LONG_WRITE, (unsigned long)(uintptr_t)SEQ_TEST_ON,
			 ARRAY_SIZE(SEQ_TEST_ON)) < 0)
		dsim_err("fail to write SEQ_TEST_ON command.\n");

	/* DSC related configuration */
	if (dsim_wr_data(id, MIPI_DSI_DSC_PRA, (unsigned long)(uintptr_t)SEQ_DSC_EN[0], 0) < 0)
		dsim_err("fail to write SEQ_DSC_EN command.\n");

	idx = s6e3hab_find_idx(lcd->yres);
	if (dsim_wr_data(id, MIPI_DSI_DSC_PPS, (unsigned long)(uintptr_t)SEQ_PPS_PARAM[idx],
			 ARRAY_SIZE(SEQ_PPS_PARAM[idx])) < 0)
		dsim_err("fail to write SEQ_PPS_PARAM command.\n");

	if (dsim_wr_data(id, MIPI_DSI_DCS_LONG_WRITE, (unsigned long)(uintptr_t)SEQ_TEST_OFF,
			 ARRAY_SIZE(SEQ_TEST_OFF)) < 0)
		dsim_err("fail to write SEQ_TEST_OFF command.\n");

	if (dsim_wr_data(id, MIPI_DSI_DCS_LONG_WRITE, (unsigned long)(uintptr_t)SEQ_PASET_TABLE,
			 ARRAY_SIZE(SEQ_PASET_TABLE)) < 0)
		dsim_err("fail to write SEQ_PASET_TABLE command.\n");

	if (dsim_wr_data(id, MIPI_DSI_DCS_LONG_WRITE, (unsigned long)(uintptr_t)SEQ_CASET_TABLE,
			 ARRAY_SIZE(SEQ_CASET_TABLE)) < 0)
		dsim_err("fail to write SEQ_CASET_TABLE command.\n");

	if (dsim_wr_data(id, MIPI_DSI_DCS_LONG_WRITE, (unsigned long)(uintptr_t)SEQ_TEST_KEY_ON_F0,
			 ARRAY_SIZE(SEQ_TEST_KEY_ON_F0)) < 0)
		dsim_err("fail to write SEQ_TEST_KEY_ON_F0 command.\n");

	if (dsim_wr_data(id, MIPI_DSI_DCS_LONG_WRITE, (unsigned long)(uintptr_t)SEQ_SCALER_TABLE,
			 ARRAY_SIZE(SEQ_SCALER_TABLE)) < 0)
		dsim_err("fail to write SEQ_SCALER_TABLE command.\n");

	if (dsim_wr_data(id, MIPI_DSI_DCS_LONG_WRITE, (unsigned long)(uintptr_t)SEQ_TEST_KEY_OFF_F0,
...
void s6e3hab_lcd_init(unsigned int id, struct exynos_panel_info *lcd)
{
	...
	if (dsim_wr_data(id, MIPI_DSI_DCS_LONG_WRITE, (unsigned long)(uintptr_t)SEQ_TEST_KEY_ON_FC,
	...
	if (dsim_wr_data(id, MIPI_DSI_DCS_SHORT_WRITE,
			 (unsigned long)(uintptr_t)SEQ_SLEEP_OUT[0], 0) < 0)
		dsim_err("fail to send SEQ_SLEEP_OUT command.\n");

	mdelay(200);

	if (dsim_wr_data(id, MIPI_DSI_DCS_LONG_WRITE, (unsigned long)(uintptr_t)SEQ_TSP_HSYNC,
			 ARRAY_SIZE(SEQ_TSP_HSYNC)) < 0)
		dsim_err("fail to write SEQ_TSP_HSYNC command.\n");

	/* enable brightness control */
	if (dsim_wr_data(id, MIPI_DSI_DCS_LONG_WRITE, (unsigned long)(uintptr_t)SEQ_BRIGHT_CTL,
			 ARRAY_SIZE(SEQ_BRIGHT_CTL)) < 0)
		dsim_err("fail to write SEQ_BRIGHT_CTL command.\n");

	/* WRDISBV(51h) = 1st[7:0], 2nd[15:8] */
	if (dsim_wr_data(id, MIPI_DSI_DCS_LONG_WRITE, (unsigned long)(uintptr_t)SEQ_WRDISBV,
			 ARRAY_SIZE(SEQ_WRDISBV)) < 0)
		dsim_err("fail to write SEQ_WRDISBV command.\n");

	/* TE on */
	if (dsim_wr_data(id, MIPI_DSI_DCS_SHORT_WRITE,
			 (unsigned long)(uintptr_t)SEQ_TE_ON[0], 0) < 0)
		dsim_err("fail to send SEQ_TE_ON command.\n");

	/* ESD flag: [2]=VLIN3, [6]=VLIN1 error check*/
	if (dsim_wr_data(id, MIPI_DSI_DCS_LONG_WRITE, (unsigned long)(uintptr_t)SEQ_ESD_FG,
			 ARRAY_SIZE(SEQ_ESD_FG)) < 0)
		dsim_err("fail to write SEQ_ESD_FG command.\n");

	/* Typical high duration: 123.57 (122~125us) */
	if (dsim_wr_data(id, MIPI_DSI_DCS_LONG_WRITE, (unsigned long)(uintptr_t)SEQ_TE_START_SETTING,
	...
	if (lcd->fps == 60)
	{
		if (dsim_wr_data(id, MIPI_DSI_DCS_LONG_WRITE, (unsigned long)(uintptr_t)SEQ_VREFRESH_60FPS,
				 ARRAY_SIZE(SEQ_VREFRESH_60FPS)) < 0)
			dsim_err("fail to write SEQ_VREFRESH_60FPS command.\n");
	}
	else if (lcd->fps == 120)
	{
		if (dsim_wr_data(id, MIPI_DSI_DCS_LONG_WRITE, (unsigned long)(uintptr_t)SEQ_VREFRESH_120FPS,
	...
	if (dsim_wr_data(id, MIPI_DSI_DCS_LONG_WRITE, (unsigned long)(uintptr_t)SEQ_GAMMA_LTPS_TRANS_UPDATE,
...
{
	if (dsim_wr_data(id, MIPI_DSI_DCS_SHORT_WRITE,
			 (unsigned long)(uintptr_t)SEQ_DISPLAY_ON[0], 0) < 0)
