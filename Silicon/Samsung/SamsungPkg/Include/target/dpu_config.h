...
/*
 * NOTE: this DXE port intentionally creates exactly one framebuffer
 * (DPP/IDMA channel LOGO_DPP). There is no font/second-framebuffer
 * layer and no PIT-based logo loader here -- the framebuffer's
 * physical base address is a UEFI-allocated buffer supplied at
 * runtime by DeconDxeEntry() (see decon_set_framebuffer_base()),
 * not a fixed LK SRAM address.
 */
...
static struct decon_dt_info decon_dt = {
	.max_win	= 6,
	.dft_win	= 5,    /* single window, full-screen framebuffer */
	.dft_ch		= LOGO_DPP,
